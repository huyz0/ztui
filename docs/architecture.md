...
- **Containment guarantees** (enforced in code): auto/content-sized widgets are clamped to the space offered by their parent (`Widget.measure`); every container clips its children to the content box by default (`Widget.renderChildren`), not just scrollables — so a child can never paint over its own border or a sibling; opt out per axis with `overflow: "visible"`; wide (2-cell) glyphs that would cross a buffer/clip edge are substituted with a space (`ScreenBuffer.setCell`); raw C0/C1 control characters are never written into a cell; and `parseDimension` clamps to `>= 0` and never returns `NaN`. Together these prevent the classic "item drawn on top of another" and "garbage char breaks the grid" failures.
...
The web backend paints the composed cell grid onto a **hardware-accelerated `<canvas>`** (`driver/web/canvas-renderer.ts`, `renderBufferToCanvas`). A canvas sidesteps the whole DOM line-box problem — there is no line box, so nothing has to reconcile "box-drawing tiles at one em" with "text wants more than one em." Each cell is painted at exact pixel coordinates:

- **Backgrounds and block elements** (`█ ░ ▒ ▓`, halves, eighths) are filled rectangles — pixel-aligned and seamless (no diagonal-hatch shade glyph).
- **Box-drawing** (U+2500–U+257F) is drawn as **vector strokes** from a small `{n,e,s,w}` arm model, with `arcTo` for the rounded corners — so borders join exactly and are font-independent.
- **Text** is `fillText`, centered in each cell (wide glyphs over two cells), so emoji/CJK can't drift the grid and ascenders/descenders never clip.

The 2-D context is requested with `{ alpha: false, desynchronized: true }` (the low-latency, GPU-composited path) and scaled by `devicePixelRatio` for crisp strokes; the same draw calls run on an `OffscreenCanvas`. The renderer is self-contained, so `driver/web/canvas-client.ts` bundles it for the browser (`canvas-bundle.ts` via `Bun.build`) — the live demo serves that bundle and streams the cell grid as JSON (`serializeForCanvas`); the {@link WebInspector} injects the same bundle so headless screenshots match the demo exactly.

The **Cascadia Mono** webfont (MIT, Windows Terminal) is vendored in `resources/fonts/` for the glyphs, and the **Seti** icon font (`resources/seti/`) supplies file-type glyphs (private-use codepoints). The host-page chrome (font faces + terminal-like CSS: no window scroll, no text selection) lives in `driver/web/host-page.ts`, shared by demo and inspector.
...
- **Ctrl+C ownership**: the BunDriver no longer hard-exits on the raw `0x03` byte. It emits `ctrl+c` as a normal key and only falls back to `process.exit` if nothing marks the event handled — so the App's selection-aware copy can claim it, while a wedged/headless path still quits. On a backend whose `capabilities.ownsProcess === false` (the web canvas, served to many users), the App **never** quits on `Ctrl+C` — with nothing to copy it swallows the key, so an end user can't tear down the shared page or the server behind it.
...
- **Rendering**: selected cells are painted with the theme's `$selectionBg`/`$selectionFg`, layered under the caret.

### 5.1b Read-only Text Selection (display widgets)
Display widgets (`Syntax`, `RichText`, `Markdown` blocks, `Table` body) support mouse drag-to-select with copy-on-release. It is modelled in **logical content space** (not screen cells): each selectable widget owns its true content and reports how its rendered cells map to it, so a selection crosses widget/nesting boundaries, highlights only real content, copies the core value (no chrome), and survives scrolling.
- **Per-widget content model** (`core/selection.ts`, `SelectableWidget`): a widget exposes `selectableLines()` — its content as logical lines, the "core value" with no gutters/borders/bullets — and, each frame during `render`, registers `SelectableRun`s (`{ widget, line, y, x, cols }`) via `App.selection.addRun`, where `cols[i]` is the logical column shown at screen column `x+i` (−1 for a wide-glyph continuation). Chrome draws register no runs, which is exactly what excludes them from hit-test, highlight, and copy. `Widget.selectable=false` opts a leaf out (Markdown sets it on list bullets / rules / blockquote bars).
- **Logical selection**: `App.selection.active = { group, anchor, caret }` where each endpoint is `{ widget, line, col }`. Global order = (document-order index of the widget within `group`, line, col) — all three invariant under scroll. `group` is the outermost `selectionContainer` ancestor (`Markdown`), so the selection is bounded by the container yet free to span its children.
- **Hit-test with snap-to-content**: `pointFromScreen(x, y)` maps an exact content cell directly; otherwise it snaps to the **closest content** (nearest row first — vertical distance dominates — then nearest column edge). A press on chrome (gutter, bullet, gap) therefore anchors on the adjacent content instead of being refused; chrome itself is still never highlighted or copied, and a click that never reaches content selects nothing. Returns null only when no content is rendered at all.
- **Highlight**: a post-pass after `screen.render` paints `$selectionBg/$selectionFg` over only the run cells whose logical point lies in `[anchor, caret)` — chrome cells (no run) are never highlighted, so the user sees exactly what will be copied.
- **Full-range copy**: `copy()` walks `group` in document order; for every `SelectableWidget` between the endpoints it slices `selectableLines()` via the shared `extractSelection` (boundary widgets clamped, interior widgets taken whole) and joins with `\n`. Because it reads each widget's full content — not the buffer — content scrolled **off-screen** still copies correctly.
- **Raw-source copy (`Widget.selectionRaw`)**: a subtree built from source text records it (Markdown stamps each block — and each list item — with its `token.raw`). During copy, a subtree whose selectable content is **fully covered** emits that raw source verbatim and is skipped for leaf extraction — so copied markdown keeps its `**bold**`/`# heading`/`- list` formatting. Partially-selected boundary blocks fall back to slicing the rendered text (a raw column for a mid-block point is not recoverable through the markup transforms).
- **Auto-scroll** (`widgets/readonly-selection.ts`): while a drag is held past the viewport edge, the nearest scrollable ancestor of `group` scrolls (a 50 ms timer keeps a held-still drag moving); the logical anchor is unaffected and the caret re-derives from freshly rendered runs each step.
- **Keyboard**: `Ctrl+C` copies an active read-only selection too — the highlight persists (repeat `Ctrl+C` re-copies); `Escape` or a new press deselects, mirroring the editable selection-aware quit path.
